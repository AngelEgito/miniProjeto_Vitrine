...
import { increment, 
         ...
         add, 
         cartRemove, 
         cartClearUpdate } from '../../store/actions';
...
class Cart extends Component {
  render() {
    ...
    if(id === 'yes') { notify('Produto adicionado com sucesso!');
    ...
    const ShowCart = () => {
      ...
      return (cart.length > 1) ? cart.map((product, index) => {return <>
        ...
        <div className="footer-list-cart"><span><button onClick={() => cartClear()} className="btn-clear-cart">Esvaziar Carrinho</button></span>&nbsp;&nbsp;TOTAL R$ {totalCart(product[0].price, amount[product[0].id])}</div>
        </>
